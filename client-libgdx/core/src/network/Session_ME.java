...
public class Session_ME implements ISession {
            ...
            try {
                ...
                try {
                    ...
                    while (Session_ME.this.isConnected()) {
                        ...
                        // readMessage() already blocks on the socket. Do not add the old 100 ms
                        // per-packet throttle here; it capped the desktop client at ~10 packets/s.
    ...
    private class NetworkInit implements Runnable {
        ...
        public void doConnect(String host, int port) throws Exception {
            ...
            if (Session_ME._mSocket == null || Session_ME._mSocket.getState() != 1) {
            ...
            if (Session_ME.this.dos == null || Session_ME.this.dis == null) {
                throw new IOException("Socket streams unavailable for " + host + ":" + port);
            }

            Session_ME.this.sendThread = new Thread(Session_ME.this.sender, "Army2-Sender");
            Session_ME.this.sendThread.setDaemon(true);
            Session_ME.this.sendThread.start();
            Session_ME.this.collectorThread = new Thread(Session_ME.this.new MessageCollector((Session_ME.MessageCollector) null), "Army2-Collector");
            Session_ME.this.collectorThread.setDaemon(true);
