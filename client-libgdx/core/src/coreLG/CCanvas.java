...
public class CCanvas extends MotherCanvas implements IActionListener {
    ...
    public void update() {
        ...
        if (currentDialog != null) {
            // A desktop key must keep working after a screen opens a modal
            // dialog. The J2ME source originally dispatched dialog commands
            // only from touch/mouse release.
            ...
            if (currentDialog == null
                ...
                if (keyPressed[5]) {
                    ...
                    if (curScr.center != null && curScr.center.action != null) {
                        curScr.center.action.perform();
                    }
                }
                if (keyPressed[12]) {
                    keyPressed[12] = false;
                    if (curScr.left != null && curScr.left.action != null) {
                        curScr.left.action.perform();
                    }
                }
                if (keyPressed[13]) {
                    keyPressed[13] = false;
                    if (curScr.right != null && curScr.right.action != null) {
