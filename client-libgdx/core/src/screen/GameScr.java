...
public class GameScr extends CScreen {
    ...
    public void drawWind(mGraphics g) {
        if (Camera.mode != 0 && ModSettings.showAimHud) {
    ...
    public static void onDrawAngleBar(mGraphics g, int x, int y, int angle) {
        if (Camera.mode != 0 && ModSettings.showAimHud) {
            ...
            int shown = ModSettings.angle360 ? ModSettings.normalize360(angle) : (angle >= 90 ? 180 - angle : angle);
            String label = ModSettings.angle360 ? "360° " + shown : String.valueOf(shown);
            Font.smallFontYellow.drawString(g, label, x, y + 4, 2);
        }
    }
